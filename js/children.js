  ...
  },
  {
    ...
    dialogue: [
      ...
      },
      {
        ...
        priority: 55,
        repeatable: true,
        condition: ({ run }) => run && run.flawless,
        lines: [
          ...['I drew you tonight and the page stayed clean. No red at all.',
            'You went all the way down and nothing got to touch you.',
            'I didn\'t know that was allowed.',
            'Do it that way always. Come back with no red on the page.',
            'I\'ll keep drawing them clean if you keep coming back clean.'].map(miri),
        ],
  ...
  },
  {
    ...
    dialogue: [
      ...
      },
      {
        ...
        condition: ({ run }) => run && run.ferocious,
        lines: ({ run }) => [
          ...[`I counted! I COUNTED! ${run.biggestCombo} in a row! Ha!`,
            'It\'s like a drum. Bap-bap-bap-bap. Best song this house ever made.',
            'Oren does the counting-with-me thing. Elsie does the sad nose thing.',
            'Me? I just think it\'s the funny kind of scary.',
            'Go make the drum go again, rabbit. Louder.'].map(pip),
        ],
        effects: { child: 'pip', bondChange: 1, childTruthChange: 1 },
      },
      {
  ...
  },
  {
    ...
    dialogue: [
      ...
      },
      {
        ...
        priority: 56,
        repeatable: true,
        condition: ({ run }) => run && run.deflector,
        lines: [
          { who: '', text: 'Jude taps the board twice to make sure Mallow is watching.' },
          writes('THEY THREW THINGS AT US TOO.'),
          writes('WE NEVER THREW THEM BACK.'),
          writes('WE DID NOT KNOW WE COULD.'),
          { who: '', text: 'He almost smiles. Almost.' },
          writes('NOW THE HOUSE KNOWS IT CAN BE HURT.'),
        ],
        effects: { child: 'jude', bondChange: 1, trustChange: 1, truthChange: 1 },
      },
      {
